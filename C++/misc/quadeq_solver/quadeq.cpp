 ...
 * 	ax^2 + bx + c = 0
 * If a is 0 or if the the equation has no real roots, the 
 ...
 * are supplied as the arguments a, b, and c, and the roots are returned 
 * in x1 and x2, which are reference parameters.
 */
voit SolveQuadratic(double a, double b, double c, double & x1, double & x2) {
 ...
 * Usage: DisplayRoots(r1, r2)
 * ---------------------------
 * This function displays the values x1 and x2, which are the 
 * roots of a quadratic equation.
 */
void DisplayRoots(double x1, double x2) {
	if (x1 == x2) { cout << "There is a double root at " << x1 << endl; }
	else { cout << "The roots are " << x1 << " and " << x2 << endl; }
}